...
#Utilities
RUN DEBIAN_FRONTEND=noninteractive apt-get install -y vim less net-tools inetutils-ping curl git telnet nmap socat dnsutils netcat tree htop unzip sudo software-properties-common nano
...
#Zookeeper
RUN curl https://olex-secure.openlogic.com/content/private/5e6a6f0815e830bba705e79e4a0470fbee8a5880/zookeeper/3.4.6/zookeeper-3.4.6.tar.gz > zookeeper-3.4.6.tar.gz | tar -xzf zookeeper-3.4.6.tar.gz
#RUN tar xzf zookeeper-3.4.6.tar.gz
RUN mv zookeeper* zookeeper
RUN cd zookeeper && cp conf/zoo_sample.cfg conf/zoo.cfg

#Kafka
RUN wget http://archive.apache.org/dist/kafka/0.8.1/kafka_2.10-0.8.1.tgz && \
    tar -xvzf kafka_*.tgz && \
    rm kafka_*.tgz && \
